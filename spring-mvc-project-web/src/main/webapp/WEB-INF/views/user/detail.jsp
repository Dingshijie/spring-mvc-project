			...
			<div id="mainList" class="col-md-8 col-md-offset-2">
					<fieldset>
						...
						<h3>
							...
							<small>
							<c:if test="${user.authentication =='NOT_APPLY' }"><span>未申请认证</span></c:if>
							<c:if test="${user.authentication =='AUDITD' }"><span>已认证</span></c:if>
							<c:if test="${user.authentication =='AUDIT' }"><span>认证审核中</span></c:if>
							<c:if test="${user.authentication =='NOT_PASS' }"><span>认证未通过</span></c:if>
							</small>
							<small>|
