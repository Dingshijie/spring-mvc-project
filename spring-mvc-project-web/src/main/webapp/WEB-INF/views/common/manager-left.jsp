		...
		<ul class="nav nav-sidebar">
			...
			<li <c:if test="${param.active=='areamanager'}">class="active"</c:if> style="white-space:nowrap; display:inline-block;width: 100%"><a href="${webRoot}/area/area.html">地区管理</a>
				<ul class="nav nav-sidebar-custom col-md-10 col-md-offset-2" <c:if test="${param.active=='areamanager'}">style="display:inline-block"</c:if> >
					...
					<li <c:if test="${param.subactive=='addarea'}">class="active"</c:if> style="white-space:nowrap; display:inline;width: 100%"><a href="javascript:alert('该模块正在建设中。。。');">添加地区</a></li>
				</ul>
			</li>
			...
			<li <c:if test="${param.active=='edupubcodemanager'}">class="active"</c:if> style="white-space:nowrap; display:inline-block;width: 100%"><a href="${webRoot}/edupubcode/edupubcode.html">专业代码管理</a>
				<ul class="nav nav-sidebar-custom col-md-10 col-md-offset-2" <c:if test="${param.active=='edupubcodemanager'}">style="display:inline-block"</c:if> >
					<li <c:if test="${param.subactive=='edupubcodelist'}">class="active"</c:if> style="white-space:nowrap; display:inline;width: 100%"><a href="${webRoot}/edupubcode/edupubcode.html">专业代码列表</a></li>
					<li <c:if test="${param.subactive=='addedupubcode'}">class="active"</c:if> style="white-space:nowrap; display:inline;width: 100%"><a href="javascript:alert('该模块正在建设中。。。');">添加专业代码</a></li>
				</ul>
			</li>
